...
const ChatList = ({ userId, username }) => {
    ...
    useEffect(() => {
        ...
        if (urlChatId && chats.length > 0) {
            const chatToSelect = chats.find(chat => chat.chat_id === parseInt(urlChatId));
            if (chatToSelect && !selectedChat) {
                handleChatSelection(chatToSelect);
            }
        }
    }, [chats, urlChatId, selectedChat, handleChatSelection]);
